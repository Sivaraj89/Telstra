...
class TelstraTests: XCTestCase {
    ...
    var responseDict :Dictionary<AnyHashable, Any> = [:]
    
    func testresponseDict(){
        ...
        XCTAssertNotNil(dict,"dict is empty")
        responseDict = dict!
    }
    
    func testJsonArray(){
        self.testresponseDict()
        let parse = JsonParser()
        let anarray = parse.parseObj(dict: responseDict)
        XCTAssertNotNil(anarray)
        XCTAssertEqual(anarray.count, 12)
